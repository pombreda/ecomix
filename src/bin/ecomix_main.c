...
typedef struct _Ecomix_App {
   ...
   magic_t magic;
   Image_Entry ie;          /* fake Image Entry for fmemopen */
   Ecomix_Arch_Info *info;  
   char *dir;               /*  Directoory of archive */
...
static void ecomix_key_down_callback(void *data, Evas *e, Evas_Object *o, void *einfo)
{
   ...
   if(key) {
	 ...
	 ocur = -1;
      } else if((!ecomix->info && ocur < ecomix->current) || (ecomix->info && ecomix->current >= eina_list_count(ecomix->info->list))) {
	 ecomix->current = change_archive(ecomix, 1, key->keyname);
	 ocur = -1;
      }
      
      if(ocur != ecomix->current) {
	 while(res == 0 && ecomix->info && eina_list_count(ecomix->info->list) > 0) {
...
static void display_image(Ecomix_App *ecomix, const Ecomix_Buffer_Load *load, const char *file, const char *key, void *buf, size_t size) {
   ...
   if(ie->surface && ie->w && ie->h) 
   {
      ...
      // evas_object_image_fill_set(o, 0, 0, ie->w, ie->h);
      // evas_object_resize(o, ie->w, ie->h);
...
static Ecomix_App *ecomix_app_init(Ecomix_App *ecomix) {
   ...
   // ecore_evas_avoid_damage_set(ecomix->ee, ECORE_EVAS_AVOID_DAMAGE_EXPOSE);
   ...
   /* There should be another way to bind key,, no ? */
   mask = 0;
   evas_object_key_grab(ecomix->container, "Up", mask, ~mask, 0);
   mask = 0;
   evas_object_key_grab(ecomix->container, "Down", mask, ~mask, 0);
   mask = 0;
   evas_object_key_grab(ecomix->container, "space", mask, ~mask, 0);
   mask = 0;
   evas_object_key_grab(ecomix->container, "KP_Enter", mask, ~mask, 0);
   mask = 0;
   evas_object_key_grab(ecomix->container, "Prior", mask, ~mask, 0);
   mask = 0;
   evas_object_key_grab(ecomix->container, "Next", mask, ~mask, 0);
   mask = 0;
   evas_object_key_grab(ecomix->container, "End", mask, ~mask, 0);
   mask = 0;
   evas_object_key_grab(ecomix->container, "Home", mask, ~mask, 0);
   mask = 0;
   evas_object_key_grab(ecomix->container, "Left", mask, ~mask, 0);
   mask = 0;
   evas_object_key_grab(ecomix->container, "Right", mask, ~mask, 0);
   mask = 0;
   evas_object_key_grab(ecomix->container, "h", mask, ~mask, 0);
   mask = 0;
   evas_object_key_grab(ecomix->container, "v", mask, ~mask, 0);
   mask = 0;
   evas_object_key_grab(ecomix->container, "f", mask, ~mask, 0);
   mask = 0;
   evas_object_key_grab(ecomix->container, "n", mask, ~mask, 0);
   mask = 0;
   evas_object_key_grab(ecomix->container, "q", mask, ~mask, 0);
   mask = 0;
   evas_object_key_grab(ecomix->container, "s", mask, ~mask, 0);
   mask = 0;
   evas_object_key_grab(ecomix->container, "r", mask, ~mask, 0);
   mask = evas_key_modifier_mask_get(ecomix->evas, "Shift");
   evas_object_key_grab(ecomix->container, "r", mask, ~mask, 0);
...
static void ecomix_usage(const char *cmd) 
{
   ...
   prog = basename(cmd);
   printf("A mimimalist image viewer for archive (tar, zip, cpio, rar, 7z) without extracting file to disk.\n\
\n\
...
\n\
Note:\n\
The command line tolols 7z, unrar, lha need to be installed to view content of archive not supported by libarchive.\n\
\n\
\n", prog);
