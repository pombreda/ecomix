...
static int _pmaps_buffer_open(Pmaps_Buffer *b);
...
int
ecomix_image_load_file_head_pmaps(Image_Entry *ie, const char *file,
				const char *key)
{
   Pmaps_Buffer b;
...
int
ecomix_image_load_fmem_head_pmaps(Image_Entry *ie, const char *file,
				const char *key, void *buf, size_t size)
{
   ...
   b.file = fmemopen(buf, size, "rb");
   if(! b.file) return 0;

   if (!_pmaps_buffer_open(&b))
     {
	pmaps_buffer_close(&b);
	return 0;
     }
...
int
ecomix_image_load_file_data_pmaps(Image_Entry *ie, const char *file,
				const char *key)
{
   Pmaps_Buffer b;
   int pixels;
   ...
   ecomix_cache_image_surface_alloc(ie, b.w, b.h);
   if (!ie->surface)
     {
	pmaps_buffer_close(&b);
	return 0;
...
int
ecomix_image_load_fmem_data_pmaps(Image_Entry *ie, const char *file,
				const char *key, void *buf, size_t size)
{
   Pmaps_Buffer b;
   int pixels;
...
/* internal used functions */
static int
_pmaps_buffer_open(Pmaps_Buffer *b)
{
   size_t len;
