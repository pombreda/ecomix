...
static int
evas_image_load_file_xpm(Image_Entry *ie, FILE *f, const char *key, int load_data)
{
   DATA32             *ptr, *end;
   ...
   while (!done)
     {
			...
			* that most xpm loaders anyway */
                                      ...
                                      for (j = 0; j < cpp; j++, i++)
					{
					   ...
					   if (col[j] < 32) col[j] = 32;
                                      ...
                                      for (j = 0; j < ncolors; j++)
                                        {
                                           if (!strcmp((char *)col, (char *)cmap[j].str))
                                             {
                                                if (cmap[j].transp)
                                                  {
        ...
        if (((ptr) && ((ptr - (DATA32 *)ie->surface) >= (w * h * sizeof(DATA32)))) ||
...
int
ecomix_image_load_file_head_xpm(Image_Entry *ie, const char *file, const char *key)
{
   FILE *f = NULL;
   if(! file) return 0;
   f = fopen(file, "rb");
   return evas_image_load_file_xpm(ie, f, key, 0);
}

int
ecomix_image_load_file_data_xpm(Image_Entry *ie, const char *file, const char *key)
{
   FILE *f = NULL;
   if(! file) return 0;
   f = fopen(file, "rb");
   return evas_image_load_file_xpm(ie, f, key, 1);
}
